...
def check_qdrant_health() -> bool:
    try:
        with urllib.request.urlopen(QDRANT_HEALTH_URL, timeout=2) as response:
...
# PROCESS CONTROL
# ======================================
def kill_process(pid_file: Path, timeout: int = 5) -> None:
    if not pid_file.exists():
        return
...
def scorched_earth() -> None:
    ...
    kill_process(QDRANT_PID_FILE)
    kill_process(MEMORY_PID_FILE)
