...
This document defines the multi-layered testing strategy for the Qwen Code stack. We employ a "Testing Pyramid" approach to ensure high confidence in system stability, from individual component logic to full end-to-end user workflows.
...
4.  **Verification Contract**: Every test must follow the `S-VERIFY` protocol: `Action` $\rightarrow$ `Verification Tool` $\rightarrow$ `Pass/Fail`.
