...
def test_test_engineer_no_fix_constraint() -> None:
    """Verify that the test-engineer agent is forbidden from using write tools."""
    engine = PolicyEngine()

    policy = engine.get_permissions(intent="Surgical Correction", agent_name="test-engineer")
...
def test_developer_can_write() -> None:
    ...
    policy = engine.get_permissions(intent="Surgical Correction", agent_name="logic-implementer")
...
def test_intent_based_write_restriction() -> None:
    ...
    policy = engine.get_permissions(intent="Exploratory Analysis", agent_name="logic-implementer")
