...
def test_rejects_invalid_permission_mode() -> None:
    with pytest.raises(ValidationError, match="Invalid permission_mode"):
        validate_query_options(QueryOptions.from_mapping({"permission_mode": "unsafe-mode"}))
...
def test_from_mapping_rejects_sync_can_use_tool() -> None:
    def can_use_tool(tool_name: object, tool_input: object, context: object) -> dict[str, object]:
...
def test_rejects_empty_qwen_executable_path() -> None:
    with pytest.raises(ValidationError, match="path_to_qwen_executable cannot be empty"):
