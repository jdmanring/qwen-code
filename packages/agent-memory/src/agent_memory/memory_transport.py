...
class UDSReadStream:
    ...
    async def _read_loop(self) -> None:
        ...
        try:
            while True:
                # Read until newline as per the serialization requirement
                ...
                try:
                    # Decode and parse the JSON wire format
                    data = json.loads(line.decode("utf-8"))
                    # Wrap in SessionMessage to avoid AttributeError in consumer
            ...
            # Normal shutdown via __aexit__
            raise
        except (OSError, RuntimeError) as e:
            logger.exception(f"Unexpected error in UDS read loop: {e}")
        finally:
            # Signal the receiver that no more messages are coming
            await self._queue.put(None)
