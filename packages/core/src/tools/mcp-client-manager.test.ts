...
vi.mock('./mcp-client.js', async () => {
  ...
  return {
    ...
    populateMcpServerCommand: vi.fn(function(servers) { return servers; }),
  };
});
...
describe('McpClientManager', () => {
  ...
  it('should discover tools from all servers', async () => {
    ...
    vi.mocked(McpClient).mockImplementation(function() { return mockedMcpClient as unknown as McpClient; });
    const mockConfig = {
  ...
  it('should coalesce concurrent discovery for the same server', async () => {
    ...
    const firstClient = {
      ...
      disconnect: vi.fn(function() { return disconnectPromise; }),
      getStatus: vi.fn(),
    };
  ...
  it('should clear in-flight discovery tracking when stopping', async () => {
    ...
    const mockedMcpClient = {
      connect: vi.fn(function() { return connectPromise; }),
      discover: vi.fn().mockResolvedValue(undefined),
      disconnect: vi.fn().mockResolvedValue(undefined),
      getStatus: vi.fn(),
    };
  ...
  it('discoverAllMcpToolsIncremental emits the trailing mcp-client-update after COMPLETED', async () => {
    ...
    > = [];
    const events = {
      emit: vi.fn(function(eventName: string) {
...
describe('McpClientManager — PR 14 guardrails', () => {
  ...
  function makeConnectedMcpClientMock() {
    ...
    return {
      ...
      discover: vi.fn().mockResolvedValue(undefined),
      disconnect: vi.fn().mockResolvedValue(undefined),
      getStatus: vi.fn(function() { return state.status; }),
  ...
  it('off mode does not reserve any slot', async () => {
    vi.mocked(McpClient).mockImplementation(
      function() { return makeConnectedMcpClientMock() as unknown as McpClient; },
    );
    const config = configWithServers({
      a: { command: 'node' },
      b: { command: 'node' },
  ...
  it('connect() failure in readResource releases the slot AND re-throws (wenshao C3)', async () => {
    ...
    vi.mocked(McpClient).mockImplementation(
      function() { return {
          ...
          disconnect: vi.fn().mockResolvedValue(undefined),
          getStatus: vi.fn(),
          readResource: vi.fn().mockImplementation(function() {
  ...
  it('runWithDiscoveryTimeout timeout handler releases the budget slot (wenshao R5 line 956)', async () => {
    ...
    vi.mocked(McpClient).mockImplementation(
      function() { return {
          connect: vi.fn(function() { return new Promise(() => {}); }),
          discover: vi.fn(),
          disconnect: vi.fn().mockResolvedValue(undefined),
          getStatus: vi.fn(),
        }; } as unknown as McpClient,
