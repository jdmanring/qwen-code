...
- **Value Framing**: Rewrite PR titles and descriptions to lead with the *value* to the upstream maintainer rather than the technical action (e.g., "Reduce CI critical path" instead of "Upgrade Vite").
...
- **Descriptive Naming**: Ensure names clearly describe the *effect* of the change (e.g., `feat/typescript-6-upgrade` instead of `contribute/phase-m`).
