...
if not _settings_path.exists():
    ...
    print(rf"  {SKIP}    \_- IntentClassifier")
    print(rf"  {SKIP}    \_- TaskDecomposer")
    print(rf"  {SKIP}    \_- JobStateManager")
    print(rf"  {SKIP}    \_- CommandManager")
    print(rf"  {SKIP}    \_- ExecutionProfileSelector (profiles loaded)")
else:
    check("ControlPlane() instantiates", _instantiate_control_plane)
    check(r"  \_- IntentClassifier", _check_cp_classifier)
    check(r"  \_- TaskDecomposer", _check_cp_decomposer)
    check(r"  \_- JobStateManager", _check_cp_jsm)
    check(r"  \_- CommandManager", _check_cp_command_manager)
    check(r"  \_- ExecutionProfileSelector (profiles loaded)", _check_cp_profile_selector)
