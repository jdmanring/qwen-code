#!/usr/bin/env python3
"""
Megalonyx stack validation.

Validates:
- PyTorch CUDA runtime
- vLLM import
- Qdrant connectivity
- Embedding model loading and generation
- Qdrant collection create / insert / retrieve
- End-to-end RAG loop

Python deps (PyTorch, sentence-transformers, vLLM) are managed via the uv
workspace. Run with: uv run python tooling/validators/validate_stack.py
"""
...
COLLECTION_NAME = "megalonyx_stack_validation"
...
try:
    ...
    for doc, vec in zip(TEST_DOCUMENTS, embeddings, strict=True):
        points.append(
            PointStruct(
                id=str(uuid.uuid4()),
                vector=vec.tolist(),
...
try:
    ...
    top = results[0]
    payload = (top.payload or {}).get("text", "")
